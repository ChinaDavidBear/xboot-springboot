        ...
        <Card>
          ...
          <Row type="flex" justify="start" class="code-row-bg">
            <Col span="6">
              ...
              <div class="tree-bar">
                <Tree ref="tree" :data="data" show-checkbox @on-check-change="changeSelect" :check-strictly="true" @on-select-change="selectTree"></Tree>
              </div>
              ...
              <Form ref="menuForm" :model="menuForm" :label-width="85" :rules="menuFormValidate">
                ...
                <FormItem label="按钮权限类型" prop="buttonType" v-if="menuForm.type===1">
                  <Select v-model="menuForm.buttonType" placeholder="请选择或输入搜索" filterable clearable>
                    <Option v-for="(item, i) in dcitPermissions" :key="i" :value="item.value">{{item.label}}</Option>
                  </Select>
                </FormItem>
...
export default {
  ...
  methods: {
    ...
    getDictPermissions() {
      findDictByType({type:'button_type'}).then(res => {
        if (res.success) {
    ...
    search() {
      if (this.searchKey) {
        this.loading = true;
        searchMenu({ title: this.searchKey }).then(res => {
          this.loading = false;
          if (res.success) {
            this.data = res.result;
          }
    ...
    submitEdit() {
      this.$refs.menuForm.validate(valid => {
        if (valid) {
          ...
          editMenu(JSON.stringify(this.menuForm)).then(res => {
